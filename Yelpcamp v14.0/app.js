...
app.use(flash()); // This line should come before our passport configuration
// For setting up flash we need to configure the session and cookie parser but since we have already done that with express we don't need to do that again

app.locals.moment = require("moment");// This line should come before our passport configuration
// Now moment is available for use in all of our view files via the variable named moment
...
app.use(function(req, res, next){
	...
	res.locals.error = req.flash("error"); //If there is any error we will store it inside our error variable
	res.locals.success = req.flash("success");//If there is success we will store it inside our success variable
	next();
});
